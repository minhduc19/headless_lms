...
<div class="row" id='<?php echo $arrParam['action'] ?>'>
    ...
    <div class="col-md-12">
        ...
        <div class="portlet">
			...
			<?php echo $this->element('admins/search/search_course'); ?>
                        <div class="row">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="datatable_products">
                                    <thead>
                                        <tr role="row" class="heading">
                                            ...
                                            <th>
                                                <?php echo $this->Filter->sort_link(__('Featured'), "$model_name.featured") ?>
                                            </th>
                                            <th>
                                    ...
                                    if (!empty($items)) {
                                        foreach ($items as $item) {
                                            ...
                                            <tr role="row" class="filter">
                                                ...
                                                <td><?php echo ($item[$model_name]['featured']) ? '<span class="badge badge-success">On</span>' : '<span class="badge badge-danger">Off</span>'; ?></td>
                                                <td><?php echo $item[$model_name]['sort_order']; ?></td>
                                                <td><?php echo $item[$model_name]['short_description']; ?></td>
                                                <td class="action-control">
                                                    <?php
                                                    echo $this->Html->link('<i class="fa fa-leanpub"></i>' . __('&nbspLessons'), array('controller' => 'lessons', 'action' => 'index', '?' => ['course_id' => $item[$model_name]['id']]), array('class' => 'btn default btn-xs green', 'div' => false, 'label' => false, 'escape' => false, 'title' => __('Detail')));
                                                    ...
                                                    echo $this->Html->link('<i class="fa fa-trash-o"></i>' . __('&nbspDelete'), array('action' => 'admin_delete', $item[$model_name]['id']), array('class' => 'btn default btn-xs btn-calling-ajax', 'data-confirm' => __('Are you sure want to delete this Course?'), 'data-confirmlevel2' => __('Delete all related lessons?'), 'escape' => false, 'data-container' => "#{$arrParam['action']}-ajax-message"));
                                                    ?>
                                                </td>
                                                <!-- END CHANGE HERE -->
                                            </tr>
...
<?php echo $this->HtmlExtend->script('plugins/a_plugins.js?v=1') ?>
<script>
    $(function() {
